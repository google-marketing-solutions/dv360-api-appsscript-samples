 ...
 */
  ...
  download() {
    ...
    if (entities.length > 0) {
      const rowsData = entities.map((result) =>
        this.buildSheetRowData_(result)
      );
      SheetUtil.appendOutputToRange(
          this.sheetConfig_.name,
  ...
  fillTargetingOptions_(entities) {
    ...
    if (targetingOptionsColumnNumber > 0) {
      entities.forEach((entity, index) => {
        ...
        SheetUtil.writeCellValue(this.sheetConfig_.name,
            rowNumber,
            targetingOptionsColumnNumber,
            value);
      });
    }
  }
  ...
  getAllData() {
    ...
    const rawDataColumnIndex = this.headerFields_
        .indexOf(this.rawDataColumnName_);
  ...
  uploadChanges() {
    ...
    this.createNewEntities_(rowsData.filter((data) =>
      data.action === 'CREATE'));
    this.updateData_(rowsData.filter((data) =>
      data.action === 'MODIFY'));
    this.deleteEntries_(rowsData.filter((data) =>
      data.action === 'DELETE'));
  }
  ...
  readAllEntitiesFromSheet_() {
    const dataRows = SheetUtil.readDataRows(this.sheetConfig_);
    const actionColumnIndex = this.headerFields_.
        indexOf(this.actionColumnName_);
    const targetingOptionsColumnIndex = this.headerFields_.
        indexOf(this.targetingOptionsColumnName_);
 ...
 */
  ...
  // read targeting options
    if (targetingOptionsData != null &&
    targetingOptionsData !== '' &&
    targetingOptionsData !== this.targetingOptionsPlaceholder_) {
      return JSON.parse(targetingOptionsData)
          .map((to) => {
  ...
  createNewEntities_(rowsData) {
    rowsData.forEach((data) => {
      try {
        ...
        const newTargetingOptions = this.updateTargetingOptions_(newEntity,
            data.targetingOptions);
        this.modificationCallback_(newEntity, data.sheetIndex);
        this.writeOutTargetingOptions(
            JSON.stringify(newTargetingOptions),
            data.sheetIndex);
      } catch (error) {
   ...
   * @param {number} sheetIndex where to put the updated targeting options
   *  in sheet
   * @return {!Array<!AssignedTargetingOption>}
   */
  updateTargetingOptions_(entity, targetingOptions) {
    if (entity.updateTargetingOptions != null) {
      const newTargetingOptions = this.prepareTargetingOptions_(
          targetingOptions,
          entity);
  ...
  updateData_(objectData) {
    objectData.forEach((data) => {
      try {
        ...
        const updatedTargetingOptions = this.updateTargetingOptions_(
            updatedEntity,
            data.targetingOptions);
        this.modificationCallback_(updatedEntity, data.sheetIndex);
        this.writeOutTargetingOptions(
            JSON.stringify(updatedTargetingOptions),
            data.sheetIndex);
      } catch (error) {
        this.handleAPIError_(data, error);
      }
  ...
  deleteEntries_(objectData) {
    ...
    objectData.forEach((data) => {
      try {
        ...
        this.writeIntoSheetRow(rowNumber,
            this.getLogsColumnNumber_(),
            ['DELETED']);
      } catch (error) {
        this.handleAPIError_(data, error);
      }

 ...
 */
  ...
  download() {
    SheetUtil.clearRange(
        ...
        this.sheetConfig_.rangeStartRow,
  ...
  getHandler_(result) {
    ...
    const rowsData = result[this.sheetConfig_.apiFieldName].map((resultRow) =>
      this.buildSheetRowData_(resultRow));
  ...
  createNewEntities_(objectData) {
    objectData.filter(this.isDataNew_)
        .forEach((data) => {
          ApiUtil.executeApiCreateRequest(
              this.buildCreateUri_(),
              data.payload,
  ...
  patchModifiedData_(objectData) {
    objectData.filter(this.isDataUpdated_)
        .forEach((data) => {
          ApiUtil.executeApiPatchRequest(
              this.buildPatchUri_(data),
              data.payload,
              (response) => this.modificationCallback_(data, response),
              (response) => this.modificationErrback_(data, response));
        });
  ...
  deleteMarkedEntries_(objectData) {
    objectData.filter(this.isDataMarkedForDeletion_)
        .forEach((data) => {
          ApiUtil.executeApiDeleteRequest(
              this.buildDeleteUri_(data),
              data.payload,
              (response) => this.modificationCallback_(data, response),
              (response) => this.modificationErrback_(data, response));
        });
  ...
  modificationCallback_(data, response) {
    ...
    this.writeIntoSheetRow(
        rowNumber,
        headerFields_.indexOf('_rawData') + 1,
        JSON.stringify(response)
    );
    const primaryIdFieldName = this.headerFields_[
        this.sheetConfig_.primaryIdCol - 1
    ];
    const primaryIdValue = response[primaryIdFieldName];
    if (primaryIdValue !== null && primaryIdValue !== 'undefined') {
      this.writeIntoSheetRow(
          rowNumber,
          this.sheetConfig_.primaryIdCol,
          primaryIdValue);
    }
   ...
   */
  writeIntoSheetRow(rowIndex, colIndex, message) {
  ...
  patchMask_(payloadObject, rawData) {
    ...
    const changedFields = Util.listDottedFields(payloadObject).filter(
        (field) =>
